...
def test_playtime_rules_partial_grant_lists_all_active_limiters(tmp_path: Path) -> None:
    rules = _build_rules(
        tmp_path,
    ...
    rules._store.set_consumed_break_debt(CHILD_PHONE, 10, now)

    decision = rules.evaluate_request(60)
    assert decision.allowed is True
    assert decision.minutes_granted == 20
    reason_lines = decision.reason.splitlines()
    assert len(reason_lines) == 4
    assert "requested 1h" in reason_lines[0]
    assert "bank" in reason_lines[1].lower()
    assert "break balance" in reason_lines[2].lower()
    assert "next blackout" in reason_lines[3].lower()
...
def test_playtime_rules_break_balance_maxed(tmp_path: Path) -> None:
    store = build_store(tmp_path)
    settings = build_settings(tmp_path)
    rules = PlaytimeRules(CHILD_PHONE, settings, store, profile_provider=lambda: settings.default_rule_profile)
    ...
    store.set_consumed_break_debt(CHILD_PHONE, 180, now)
...
def test_playtime_rules_set_break_balance(tmp_path: Path) -> None:
    store = build_store(tmp_path)
    settings = build_settings(tmp_path, break_balance_max_minutes=180)
    rules = PlaytimeRules(CHILD_PHONE, settings, store, profile_provider=lambda: settings.default_rule_profile)

    now = datetime.now(timezone.utc)
    store.set_consumed_break_debt(CHILD_PHONE, 30, now)

    _result = rules.set_consumed_break_debt(90)
    assert store.get_consumed_break_debt(CHILD_PHONE)[0] == 90

    _result_capped = rules.set_consumed_break_debt(999)
    assert store.get_consumed_break_debt(CHILD_PHONE)[0] == 180
